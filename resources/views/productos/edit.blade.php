...
<div class="container-fluid px-4">
  ...
  <form action="{{ route('productos.update', $producto) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-3">
      ...
      <div class="col-md-6">
        ...
        >
          @foreach ($presentaciones as $presentacione)
            <option value="{{ $presentacione->id }}" {{ old('presentacione_id', $producto->presentacione_id) == $presentacione->id ? 'selected' : '' }}>{{ $presentacione->caracteristica->nombre }}</option>
          @endforeach
        </select>
      ...
      @php
        $productoCategorias = $producto->categorias->pluck('id')->toArray() ?? []
      @endphp
      ...
      <div class="col-md-6">
        ...
        >
          @foreach ($categorias as $categoria)
            <option value="{{ $categoria->id }}" {{ in_array($categoria->id, old('categorias', $productoCategorias)) ? 'selected' : '' }}>{{ $categoria->caracteristica->nombre }}</option>
          @endforeach
        </select>
        @error('categorias')
          <small class="text-danger">{{ $message }}</small>
